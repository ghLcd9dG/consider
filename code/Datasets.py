...
def convert_examples_to_features(item, lang=None, LT=False, NL_LT=False):
    ...
    if args.model_type == "graphcodebert":
        ...
        if LT:
            ...
            code_tokens = code_tokens[:args.code_length +
                                      args.data_flow_length-8-min(len(dfg), args.data_flow_length)]
            code_tokens = [tokenizer.cls_token] + \
                LANGS_TOKENS+code_tokens+[tokenizer.sep_token]
        else:
            code_tokens = code_tokens[:args.code_length +
        ...
        if MultiLT:
            ...
            position_idx += [i+tokenizer.pad_token_id +
                             1 for i in range(len(code_tokens)-6)]
        else:
            # pad_token_id is 1, start from 2
            position_idx = [i+tokenizer.pad_token_id +
        ...
        if args.tokenizer_type == "dfg":
            parser = parsers[lang]
            code_tokens, _ = extract_dataflow(
                js['original_string'], parser, lang)
            code_tokens = [tokenizer.tokenize(
                '@ '+x)[1:] if idx != 0 else tokenizer.tokenize(x) for idx, x in enumerate(code_tokens)]
            ori2cur_pos = {}
        ...
        if LT:
            ...
            code_tokens = [tokenizer.cls_token, "<encoder-only>",
                           tokenizer.sep_token]+['[LS]']+code_tokens+[tokenizer.sep_token]
        elif MultiLT:
            code_tokens = tokenizer.tokenize(code)[:args.code_length-10]
            code_tokens = [tokenizer.cls_token]+LANGS_TOKENS+["<encoder-only>",
        ...
        if MultiLT:
            ...
            # pad_token_id is 1, start from 2
            position_idx += [i+tokenizer.pad_token_id +
                             1 for i in range(len(code_ids)-8)]
        else:
            position_idx = [i + tokenizer.pad_token_id +
                            1 for i in range(args.code_length)]
        dfg_to_code, dfg_to_dfg = None, None
    ...
    if args.model_type == "unixcoder":
        ...
        if NL_LT:
            ...
            nl_tokens = [tokenizer.cls_token, "<encoder-only>",
                         tokenizer.sep_token]+['[LS]']+nl_tokens+[tokenizer.sep_token]
        else:
        ...
        if NL_LT:
            ...
            nl_tokens = [tokenizer.cls_token] + \
                ['[LS]']+nl_tokens+[tokenizer.sep_token]
        else:
            ...
            if ML and 'train' in file_path:
                for lang in LANGS:
                    ...
                    file_examples = pool.map(convert_examples_to_features_with_lang(
                        lang, LT=args.LT, model_type=args.model_type), tqdm(data, total=len(data)))
            ...
            for idx, nodes in enumerate(self.examples[item].dfg_to_dfg):
                for a in nodes:
                    if a+node_index < len(self.examples[item].position_idx):
                        attn_mask[idx+node_index, a +
                                  node_index] = True   # 为何使用的单向的边
...
class SamplerIterator():
    def __init__(self, examples, args, num_batches, batchsize=32, alpha=0.5, beta=1, mode="loader"):
        ...
        self.lang_datasets = {'go': [], 'java': [],
                              'javascript': [], 'php': [], 'python': [], 'ruby': []}
        self.lang_P = {}
        self.lang_nums = {'go': 0, 'java': 0,
                          'javascript': 0, 'php': 0, 'python': 0, 'ruby': 0}
        ...
        self.lang_class_P = np.array(
            [len(self.lang_datasets[lang]) for lang in LANGS])
    ...
    def __iter__(self):
        for _ in range(self.num_batches):
            ...
            selected_lang = np.random.choice(
                np.arange(6), size=1, p=self.lang_class_P).squeeze()
            ...
            for lang, nums in load_lang_nums.items():
                if nums > 0:
                    ...
                    choiceIdx = np.random.choice(np.arange(
                        self.lang_nums[lang]), size=nums, p=self.lang_P[lang]/sum(self.lang_P[lang]), replace=False)
            ...
            code_ids_batch, position_idx_batch, nl_ids_batch, lang_batch = torch.stack(
                code_ids_batch), torch.stack(position_idx_batch), torch.stack(nl_ids_batch), torch.stack(lang_batch)
            if "graphcodebert" in self.args.model_name_or_path:
