...
LANGS_TOKENS = ['[Go]', '[Java]', '[JavaScript]',
                '[PHP]', '[Python]', '[Ruby]']
temperture = 1
rounds = 0
...
def train(args, model, tokenizer, pool, ML=False, DCmodel=None, CRModel=None):
    ...
    train_dataset = TextDataset(
        tokenizer, args, args.train_data_file, pool, ML=ML, NL_LT=args.NL_LT)
    if args.BSampler:
        start_idx = 0
        indice_idx = []
        logger.info(f"train_dataset.lang_len: {train_dataset.lang_len}")
        for lang in LANGS:
            indice_list = random.sample(range(
                start_idx, start_idx + train_dataset.lang_len[lang]), k=train_dataset.lang_len[lang])
        ...
        seqIdx = [[indice_idx[i*BS + j]
                   for j in range(BS)] for i in range(LEN // BS)]
    ...
    if args.MCL and SamplerData:
        SI = SamplerIterator(train_dataset, args, num_batches=len(
            train_dataloader) * 2, batchsize=args.train_batch_size)
        sampler_dataset = SamplerIterableDataset(SI)
        sampler_dataloader = DataLoaderX(
            sampler_dataset, batch_size=None, pin_memory=True, num_workers=0)
    ...
    if args.Commonalities:
        ...
        scheduler = get_linear_schedule_with_warmup(
            CRoptim, num_warmup_steps=0, num_training_steps=num_training_steps)
    else:
    ...
    for idx in range(args.num_train_epochs):
        if args.MCL:
            ...
            train_sampler = BatchSampler(
                randomIdx, batch_size=BS, drop_last=False)
            train_dataloader = DataLoader(
                train_dataset, batch_sampler=train_sampler, num_workers=4)
            if args.MCL:
                ml_train_sampler = RandomSampler(train_dataset)
                ml_train_dataloader = DataLoader(
                    train_dataset, sampler=ml_train_sampler, batch_size=args.train_batch_size, num_workers=4)
        ...
        for step, batch in enumerate(sampler_dataloader):
            ...
            for k in range(step_nums):
                ...
                with autocast():
                    ...
                    if args.Commonalities:
                        results = CRModel(code_inputs=code_inputs, attn_mask=attn_mask, position_idx=position_idx,
                                          nl_inputs=nl_inputs, lang_labels=lang_labels, step=idx, nl_length=None)
                        query_emb, LI_emb, LS_emb, fusionEmb, adv_loss, cs_loss, length_loss = results["query_emb"], results[
                            "LI_emb"], results["LS_emb"], results["fusion_emb"], results["adv_loss"], results["cs_loss"], results["length_loss"]

                        if hasattr(fusionEmb, 'last_hidden_state'):
                            code_vecs = fusionEmb.last_hidden_state[:, :2, :]
                            # normalize
                            query_emb = torch.nn.functional.normalize(
                                query_emb, p=2, dim=1)
                            CLS_emb = code_vecs[:, 0, :]
                            LT_emb = code_vecs[:, 1, :]
                            scores = torch.max(torch.stack([torch.einsum(
                                "ab,cb->ac", query_emb, CLS_emb), torch.einsum("ab,cb->ac", query_emb, LT_emb)], dim=2), dim=2)[0]
                        elif fusionEmb is not None:
                            scores = torch.einsum(
                                "ab,cb->ac", query_emb, fusionEmb)
                        else:
                            LI_scores = torch.einsum(
                                "ab,cb->ac", query_emb, LI_emb)
                        if args.n_gpu > 1:
                    ...
                    if scores is not None:
                        loss = loss_fct(
                            scores/temperture, torch.arange(code_inputs.size(0), device=args.device))
                    if LI_scores is not None:
                        LIloss = loss_fct(
                            LI_scores/temperture, torch.arange(code_inputs.size(0), device=args.device))
                        li_loss += LIloss.item()
                    else:
                        LIloss = 0
                    loss += LIloss
                ...
                tr_num += 1
                if (step+1) % 100 == 0:
                    logger.info("epoch {} step {} loss {} li_loss: {} cs_loss: {} dc_loss: {} mdc_loss: {} train_length_loss: {}".format(idx, step+1, round(tr_loss/tr_num, 5), round(
                        li_loss/tr_num, 5), round(train_cs_loss/tr_num, 5), round(train_dc_loss/tr_num, 5), round(train_mdc_loss/tr_num, 5), round(train_length_loss/tr_num, 5)))
                    tr_loss, train_cs_loss, train_dc_loss, train_mdc_loss, train_length_loss, li_loss = 0, 0, 0, 0, 0, 0
                    tr_num = 0

                # backward
                scaler.scale(loss).backward(
                    retain_graph=True if ExtraData else False)
        ...
        if ML:
            ...
            for lang in LANGS:
                ...
                result = evaluate(args, model, tokenizer, path, pool,
                                  eval_when_training=True, CRModel=CRModel, evalML=True)
            ...
            results = evaluate(args, model, tokenizer, args.eval_data_file,
                               pool, eval_when_training=True, CRModel=CRModel, evalML=True)
        for key, value in results.items():
            if key == "eval_mrr":
                logger.info("  %s = %s", key, round(value, 4))
        sampler_dataloader.dataset.ConfusionMatrix = np.array(
            results["lang_distr"])
        logger.info(
            f"sampler data distribution: {sampler_dataloader.dataset.ConfusionMatrix}")
...
def evaluate(args, model, tokenizer, file_name, pool, eval_when_training=False, CRModel=None, evalML=False):
    ...
    query_dataset = TextDataset(
        tokenizer, args, file_name, pool, ML=ML, NL_LT=args.NL_LT)
    ...
    code_dataset = TextDataset(
        tokenizer, args, codebase_file, pool, ML=ML, NL_LT=args.NL_LT)
    ...
    if evalML and os.path.exists(f"{args.model_type}_code_vecs_r{rounds}.pkl"):
        code_vecs = pickle.load(
            open(f"{args.model_type}_code_vecs_r{rounds}.pkl", "rb"))
        ...
        for batch in code_dataloader:
            ...
            with torch.no_grad():
                with autocast():
                    ...
                    if args.Commonalities:
                        results = CRModel(code_inputs=code_inputs, attn_mask=attn_mask, position_idx=position_idx,
                                          nl_inputs=None, lang_labels=lang_labels, nl_length=None, mode="test")
    ...
    if code_vecs != []:
        ...
        scores = np.maximum(np.matmul(nl_vecs, code_embs.T),
                            np.matmul(nl_vecs, lt_embs.T))
...
def main():
    ...
    if args.do_eval:
        ...
        result = evaluate(args, model, tokenizer, args.eval_data_file,
                          pool, CRModel=CRModel, evalML=args.evalML)
    ...
    if args.do_test:
        ...
        if args.Commonalities:
            ...
            result = evaluate(args, model, tokenizer,
                              args.test_data_file, pool, evalML=args.evalML)
        logger.info("***** Eval results *****")
        for key in sorted(result.keys()):
            logger.info("  %s = %s", key, str(round(result[key], 4)))
