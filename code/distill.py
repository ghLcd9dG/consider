...
LANGS_TOKENS = ['<go>', '<java>', '<javascript>', '<php>',
                '<python>', '<ruby>']  # TODO: alter 括号, 更改在输入中的位置
# LANGS = ['python', 'ruby']
temperture = 1
...
def convert_examples_to_features(item, lang=None, LT=False, model_type="graphcodebert"):
    ...
    if model_type == "graphcodebert":
        ...
        code_tokens, dfg = extract_dataflow(
            js['original_string'], parser, lang)
        ...
        # pad_token_id is 1, start from 2
        ...
        position_idx += [tokenizer.pad_token_id] * \
            padding_length  # extra postion id is 1
        ...
        code_ids = tokenizer.convert_tokens_to_ids(code_tokens)
        position_idx = [i + tokenizer.pad_token_id +
                        1 for i in range(args.code_length)]
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path=None, pool=None, ML=False):
        ...
        if ML:
            if not args.LT:
                prefix = prefix + "_woLT"
            cache_file = '/'.join(file_path.split('/')
                                  [:-1]) + '/' + prefix + '.pkl'
        else:
            cache_file = '/'.join(file_path.split('/')
                                  [:-1]) + '/' + prefix + '.pkl'
        ...
        if os.path.exists(cache_file):
            ...
            self.lang_len = {'go': 167288, 'java': 164923, 'javascript': 58025,
                             'php': 241241, 'python': 251820, 'ruby': 24927}
            # for lang in LANGS:
            ...
            if ML and 'train' in file_path:
                for lang in LANGS:
                    ...
                    file_examples = pool.map(convert_examples_to_features_with_lang(
                        lang, LT=args.LT, model_type=model_type), tqdm(data, total=len(data)))
                ...
                if 'javascript' in file_path:
                    lang = 'javascript'
                logger.info(
                    "building dataset, file_path: {} lang: {}".format(file_path, lang))
                with open(file_path) as f:
                    for line in f:
                        line = line.strip()
                        js = json.loads(line)
                        data.append((js, tokenizer, args))
                self.examples = pool.map(convert_examples_to_features_with_lang(
                    lang, LT=args.LT, model_type=model_type), tqdm(data, total=len(data)))
    ...
    def load_teacher_emb(self, code_teacher_embs, nl_teacher_embs):
        for i in tqdm(range(len(self.examples)), desc="load teacher emb..."):
            ...
            item = InputFeatures(exmaple.code_tokens, exmaple.code_ids, exmaple.position_idx, exmaple.dfg_to_code, exmaple.dfg_to_dfg,
                                 exmaple.nl_tokens, exmaple.nl_ids, exmaple.url, exmaple.lang, code_teacher_embs[i], nl_teacher_embs[i])
    ...
    def __getitem__(self, item):
        if "graphcodebert" in self.args.model_name_or_path:
            ...
            node_index = sum(
                [i > 1 for i in self.examples[item].position_idx])   # code length
            # code + dfg length
            max_length = sum(
                [i != 1 for i in self.examples[item].position_idx])
...
def train(args, model, tokenizer, pool, ML=False, DCmodel=None, CRModel=None):
    ...
    teacher_datasets = [TextDataset(
        tokenizer, args, 'dataset/{}/train.jsonl'.format(lang), pool, ML=False) for lang in LANGS]
    teacher_dataloaders = []
    for dataset in teacher_datasets:
        teacher_dataloaders.append(DataLoader(
    ...
    for i in range(len(LANGS)):
        ...
        if os.path.exists(teacher_emb_path):
            with open(teacher_emb_path, 'rb') as f:
                ...
                teacher_datasets[i].load_teacher_emb(
                    code_teacher_embs, nl_teacher_embs)
            continue
        ...
        teacher_model_path = 'saved_models/teacher_model/codebert_{}.bin'.format(
            lang)
        teacher_model.load_state_dict(torch.load(
            teacher_model_path, map_location=args.device), strict=False)
        ...
        for step, batch in tqdm(enumerate(teacher_dataloaders[i]), desc="teacher eval..."):
            ...
            with torch.no_grad():
                ...
                code_vec = teacher_model(
                    code_inputs=code_inputs, attn_mask=attn_mask, position_idx=position_idx)[1]
    ...
    student_dataloader = DataLoader(
        student_datasets, batch_size=args.train_batch_size, shuffle=True, num_workers=8)

    # get optimizer and scheduler
    if args.Summarization:
        CRoptim = AdamW(CRModel.parameters(), lr=args.learning_rate, eps=1e-8)
        scheduler = get_linear_schedule_with_warmup(
            CRoptim, num_warmup_steps=0, num_training_steps=len(student_dataloader)*args.num_train_epochs)
    else:
        optimizer = AdamW(model.parameters(), lr=args.learning_rate, eps=1e-8)
        scheduler = get_linear_schedule_with_warmup(
            optimizer, num_warmup_steps=0, num_training_steps=len(student_dataloader)*args.num_train_epochs)
    ...
    logger.info("  Total optimization steps = %d", len(
        student_dataloader)*args.num_train_epochs)
    ...
    for idx in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(student_dataloader):
            ...
            with autocast():
                ...
                code_vec = model(
                    code_inputs=code_inputs, attn_mask=attn_mask, position_idx=position_idx)[1]
                teacher_code_with_student_nl = torch.einsum(
                    "ab,cb->ac", nl_vec, code_teacher_emb)
                teacher_nl_with_student_code = torch.einsum(
                    "ab,cb->ac", nl_teacher_emb, code_vec)
                scores = torch.einsum("ab,cb->ac", nl_vec, code_vec)
                # scores = scores + teacher_code_with_student_nl + teacher_nl_with_student_code

                loss_fct = CrossEntropyLoss()
                loss = loss_fct(
                    scores/temperture, torch.arange(code_inputs.size(0), device=scores.device))
                loss += loss_fct(teacher_code_with_student_nl/temperture,
                                 torch.arange(code_inputs.size(0), device=scores.device))
                loss += loss_fct(teacher_nl_with_student_code/temperture,
                                 torch.arange(code_inputs.size(0), device=scores.device))
                tr_loss += loss.item()

            tr_num += 1
            if (step+1) % 100 == 0:
                logger.info("epoch {} step {} loss {} li_loss: {} cs_loss: {} dc_loss: {} mdc_loss: {} ".format(idx, step+1, round(tr_loss/tr_num, 5),
                            round(li_loss/tr_num, 5), round(train_cs_loss/tr_num, 5), round(train_dc_loss/tr_num, 5), round(train_mdc_loss/tr_num, 5)))
        ...
        if ML:
            ...
            for lang in LANGS:
                ...
                result = evaluate(args, model, tokenizer, path,
                                  pool, eval_when_training=True, CRModel=CRModel)
        ...
        if results['eval_mrr'] > best_mrr:
            ...
            # checkpoint-best-mrr-BS70-tempAnneal-LT-CN -BS240-maxLTLI-norm
            checkpoint_prefix = 'checkpoint-best-mrr-codebert-distill'
            output_dir = os.path.join(
                args.output_dir, '{}'.format(checkpoint_prefix))
            if not os.path.exists(output_dir):
                os.makedirs(output_dir)
            if args.Summarization:
                model_to_save = CRModel.module if hasattr(
                    CRModel, 'module') else CRModel
            else:
...
def evaluate(args, model, tokenizer, file_name, pool, eval_when_training=False, CRModel=None, evalML=False):
    ...
    for batch in code_dataloader:
        ...
        with torch.no_grad():
            with autocast():
                ...
                if args.Summarization:
                    code_vec = CRModel(code_inputs=code_inputs, attn_mask=attn_mask,
                                       position_idx=position_idx, nl_inputs=None, nl_length=None, mode="test")
    ...
    if code_vecs != []:
        # n, hidden * nbs, seq, hidden = n, nbs, seq
        for i in range(CodeLength // NBS + 1):
            if i*NBS >= CodeLength:
                break
            if CLS_emb is not None:
                score = torch.max(torch.stack([torch.einsum("ab,cb->ac", nl_vecs.to(args.device), code_embs[i*NBS:(i+1)*NBS, :].to(
                    args.device)), torch.einsum("ab,cb->ac", nl_vecs.to(args.device), lt_embs[i*NBS:(i+1)*NBS, :].to(args.device))], dim=1), dim=1)[0]
            else:
                score = torch.einsum(
                    "ab,cb->ac", nl_vecs.to(args.device), code_vecs[i*NBS:(i+1)*NBS, :].to(args.device))
            # score = torch.max(score, dim=2)[0]
            scores.append(score.cpu())
        scores = torch.cat(scores, 1)
    else:
        scores = np.maximum(np.matmul(nl_vecs, code_embs.T), np.matmul(
            nl_vecs, lt_embs.T))  # b, hidden * hidden, b = b,b

    scores = scores.cpu().numpy()
    # each nl to sort from big to small scores
    sort_ids = np.argsort(
...
def main():
    ...
    # Training
    if args.do_train:
        train(args, model, tokenizer, pool, ML=True if args.lang ==
              'ML' else False, DCmodel=DCmodel, CRModel=CRModel)
    ...
    if args.do_eval:
        ...
        if args.Summarization:
            CRModel.to(args.device)
            CRModel.load_state_dict(torch.load(
                output_dir, map_location=args.device), strict=False)
        model.to(args.device)
        model.load_state_dict(torch.load(
            output_dir, map_location=args.device), strict=False)

        result = evaluate(args, model, tokenizer,
                          args.eval_data_file, pool, CRModel=CRModel)
    ...
    if args.do_test:
        ...
        if args.Summarization:
            ...
            CRModel.load_state_dict(torch.load(
                output_dir, map_location=args.device), strict=False)
            result = evaluate(args, model, tokenizer,
                              args.test_data_file, pool, CRModel=CRModel)
        else:
            model.to(args.device)
            model.load_state_dict(torch.load(
                output_dir, map_location=args.device), strict=False)
